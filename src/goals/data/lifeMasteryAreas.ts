...
export const LIFE_MASTERY_AREAS: LifeMasteryArea[] = [
  ...
  { id: "lm_mindset", label: "Mind & Beliefs", sublabel: "Thoughts, Mindset, Mentality", band: 1, color: "#6366f1", prompt: "How empowering are your thoughts and belief systems?", pillarIds: ["meaning", "vices"] },
  ...
  { id: "lm_family", label: "Family", sublabel: "Connection", band: 6, color: "#a855f7", prompt: "How connected are you with your family?", pillarIds: ["relations"] },
...
/** Does this goal feed this Blueprint area? An explicit `areaId` (Life Plan
 * per-area goal) feeds ONLY that area; otherwise the pillar mapping decides. */
export function goalFeedsArea(goal: { pillarId: string; areaId?: string | null }, areaId: string): boolean {
  if (goal.areaId) return goal.areaId === areaId
  const area = LIFE_MASTERY_AREA_MAP.get(areaId)
  return !!area && area.pillarIds.includes(goal.pillarId)
}
