...
import { GlassCard } from "../setup/GlassCard"
import {
  ORBIT_CONFIG,
  SUN_RADIUS,
  CENTER,
...
export function OrreryView({ goals }: OrreryViewProps) {
  ...
  // Derive stats from real goals
  const areaProgress = useMemo(() => computeLifeAreaProgress(goals), [goals])
  const areaProgressMap = useMemo(() => {
    const map = new Map<string, { avgProgress: number; completed: number; total: number }>()
    for (const ap of areaProgress) {
      map.set(ap.lifeArea, ap)
    }
    return map
  }, [areaProgress])
  ...
  const activeAreas = useMemo(() => {
    ...
    for (const g of goals) {
      if (!g.is_archived && g.life_area) set.add(g.life_area)
    }
    return set
  }, [goals])

  // L2 achievement badges with real progress
  ...
  const totalGoals = goals.filter(g => !g.is_archived).length
  const overallProgress = useMemo(() => {
    const active = goals.filter(g => !g.is_archived)
    if (active.length === 0) return 0
    const completed = active.filter(g => g.is_complete).length
  ...
  // Generate aurora ribbon path for an orbit
  ...
  function chargedParticleDots(radius: number, count: number): { cx: number; cy: number; r: number; opacity: number }[] {
  ...
  // Progress arc for a planet (drawn as a stroke-dasharray circle)
  function progressArcProps(radius: number, planetSize: number, progress: number) {
    const arcRadius = planetSize + 4
    const circumference = 2 * Math.PI * arcRadius
    const dashLength = (progress / 100) * circumference
    return { arcRadius, circumference, dashLength }
  }

  return (
    <div className="relative rounded-xl overflow-hidden" style={{ background: "linear-gradient(135deg, #010510 0%, #050d1a 40%, #0a1218 100%)" }}>
        ...
        <div className="mx-auto max-w-3xl">
          <div className="text-center mb-6">
            ...
            <p className="text-white/40 text-sm">
              {totalGoals} goals {"\u00B7"} {activeAreas.size} life areas {"\u00B7"} {overallProgress}% complete
            </p>
          </div>
          ...
          >
            ...
            >
              ...
              {[0, 60, 120, 180, 240, 300].map((angle, i) => (
                <g key={`geo-${i}`}>
                  <path
                    ...
                    fill="none" stroke="#00E676" strokeWidth="0.4"
                    strokeOpacity={0.04 + Math.sin(t * 0.3 + i) * 0.02}
                    strokeDasharray="3,10" filter="url(#v9c-soft-blur)"
                  />
                  <path
                    d={magneticFieldPath(CENTER * 0.9, angle, -1)}
                    fill="none" stroke="#7C4DFF" strokeWidth="0.4"
              ...
              {/* Magnetic field lines to active planets */}
              {visibleAreas.map((area) => {
                const config = ORBIT_CONFIG[area.id]
                if (!config || !activeAreas.has(area.id)) return null
                const haloColors = AURORA_HALO_COLORS[area.id] ?? ["#00E676"]
                return (
                  <g key={`field-${area.id}`}>
                    <path d={magneticFieldPath(config.radius, config.startAngle, 1)}
                      fill="none" stroke={haloColors[0]} strokeWidth="0.8"
                      strokeOpacity={0.08 + Math.sin(t * 0.5 + config.startAngle) * 0.04}
                      strokeDasharray="4,8" filter="url(#v9c-aurora-blur)"
                    />
                    <path d={magneticFieldPath(config.radius, config.startAngle, -1)}
                      fill="none" stroke={haloColors[1] ?? haloColors[0]} strokeWidth="0.6"
                      strokeOpacity={0.06 + Math.sin(t * 0.6 + config.startAngle * 1.5) * 0.03}
                      strokeDasharray="3,10" filter="url(#v9c-aurora-blur)"
                    />
                  </g>
                )
              })}

              {/* Aurora ribbon orbit paths */}
              {visibleAreas.map((area) => {
                const config = ORBIT_CONFIG[area.id]
                ...
                return (
                  <g key={`orbit-${area.id}`}>
                    {isActive && (
                      <path d={auroraOrbitPath(config.radius, 3)} fill="none"
                    ...
                    <path d={auroraOrbitPath(config.radius)} fill="none"
                    ...
                    />
                    {isActive && (
                      <path d={auroraOrbitPath(config.radius + 2, 1.5)} fill="none"
                        stroke={haloColors[1]}
                    ...
                    {isActive && (
                      <path d={auroraOrbitPath(config.radius - 2, 2.5)} fill="none"
                        stroke={haloColors[3] ?? haloColors[0]}
                        strokeWidth="0.8" strokeOpacity={0.08}
                        filter="url(#v9c-aurora-blur)"
                      />
                    )}
                    {isActive && chargedParticleDots(config.radius, 24).map((dot, di) => (
                      <circle key={`particle-${area.id}-${di}`}
              ...
              {Array.from({ length: 16 }).map((_, i) => {
                ...
                return (
                  <path key={`tendril-${i}`} d={sunTendrilPath(i, 16)}
              ...
              <text x={CENTER} y={CENTER + 2} textAnchor="middle" dominantBaseline="middle"
                fill="#e8ffe8" fontSize="8" fontWeight="700" letterSpacing="0.8"
              ...
              {visibleAreas.map((area) => {
                ...
                const haloColors = AURORA_HALO_COLORS[area.id] ?? ["#00E676", "#7C4DFF", "#FF4081", "#69F0AE"]
                const ap = areaProgressMap.get(area.id)
                const progress = ap?.avgProgress ?? 0
                ...
                const arc = progressArcProps(config.radius, config.planetSize, progress)
                ...
                return (
                  <g key={`planet-${area.id}`}
                    ...
                    <g style={{
                      ...
                      {/* Hover hit area */}
                      <circle cx={CENTER} cy={CENTER - config.radius}
                        r={config.planetSize + 20} fill="transparent" className="cursor-pointer"
                      ...
                      {isActive && (
                        <circle cx={CENTER} cy={CENTER - config.radius}
                          ...
                          fill="none" stroke={haloColors[0]} strokeWidth="1"
                      ...
                      {haloRings.map((ring, ri) => {
                        ...
                        return (
                          <circle key={`halo-${ri}`}
                            cx={CENTER} cy={CENTER - config.radius}
                      ...
                      {isActive && progress > 0 && (
                        <circle
                          ...
                          style={{ transform: `rotate(-90deg)`, transformOrigin: `${CENTER}px ${CENTER - config.radius}px` }}
                        />
                      )}
                      ...
                      <circle cx={CENTER} cy={CENTER - config.radius}
                        ...
                        filter={isActive ? (isHovered ? "url(#v9c-active-glow)" : "url(#v9c-planet-glow)") : undefined}
                      ...
                      {/* Planet equator */}
                      {isActive && (
                        <ellipse cx={CENTER} cy={CENTER - config.radius}
                          rx={config.planetSize} ry={config.planetSize * 0.25}
                      ...
                      <circle
                        ...
                        fill="white" fillOpacity={isActive ? 0.18 : 0.05}
                      />
                      {isActive && (
                        <circle
                      ...
                      <text x={CENTER} y={CENTER - config.radius + config.planetSize + 16}
                        textAnchor="middle"
                        fill={isActive ? (isHovered ? "#fff" : "rgba(255,255,255,0.8)") : "rgba(255,255,255,0.2)"}
                      ...
                      {isActive && count > 0 && (
                        <g>
                          <circle
                            ...
                            r={7} fill="rgba(5,8,16,0.85)"
                            stroke={haloColors[0]} strokeWidth="1" strokeOpacity="0.6"
                          />
                          <text
                            x={CENTER + config.planetSize - 2}
                            y={CENTER - config.radius - config.planetSize + 2.5}
                            textAnchor="middle" dominantBaseline="middle"
                            fill={haloColors[0]} fontSize="6" fontWeight="700"
                            style={{ fontFamily: "system-ui, sans-serif" }}>
                            {count}
                          </text>
                        </g>
                      ...
                      {isHovered && isActive && (
                        <g>
                          <rect
                            x={CENTER - 60} y={CENTER - config.radius - config.planetSize - 35}
                            width={120} height={28} rx={8}
                            fill="rgba(5,8,16,0.92)"
                            stroke={haloColors[0]} strokeWidth="0.8" strokeOpacity="0.6"
                          />
                          <text x={CENTER} y={CENTER - config.radius - config.planetSize - 18}
                            textAnchor="middle" fill="white" fontSize="7.5" fontWeight="500"
                            style={{ fontFamily: "system-ui, sans-serif" }}>
                            {count} goal{count !== 1 ? "s" : ""} · {progress}%
                          </text>
                        </g>
                      )}
                    </g>
          ...
          {badges.length > 0 && (
            <div className="mt-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {[...badges]
                  .sort((a, b) => (SETUP_TIER_ORDER[a.tier] ?? 4) - (SETUP_TIER_ORDER[b.tier] ?? 4))
                  .map((badge) => {
                    const tierColor = TIER_HEX[badge.tier]
                    return (
                      <GlassCard key={badge.id} glowColor={tierColor}>
                        <div className="p-3 text-center">
                          <div
                            className="size-8 rounded-full flex items-center justify-center mx-auto mb-1.5"
                            style={{ background: `${tierColor}12`, boxShadow: `0 0 12px ${tierColor}15` }}>
                            <Trophy className="size-4" style={{ color: tierColor }} />
                          </div>
                          <div className="text-xs text-white/70 leading-tight">{badge.title}</div>
                          <div className="mt-1.5 h-1.5 rounded-full bg-white/10 overflow-hidden mx-2">
                            <div
                              className="h-full rounded-full transition-all duration-500"
                              style={{ width: `${badge.progress}%`, backgroundColor: tierColor }}
                            />
                          </div>
                          <div className="text-[10px] uppercase font-medium mt-1" style={{ color: tierColor }}>
                            {badge.tier === "none" ? `${badge.progress}%` : `${badge.tier} · ${badge.progress}%`}
                          </div>
                        </div>
                      </GlassCard>
                    )
