...
/**
 * V8: Cosmic Particles + Per-Category Summary Colors
 *
 * Base: V7 Variant A (Cosmic Particles) — particle field, glassmorphism, orrery
 * Summary: V7 Variant C color system — per-category colors, stagger animations, glow-pulse
 * Badges: Sorted by tier (bronze → silver → gold → diamond → locked)
 */
...
// Per-category color map (from V7 Variant C)
...
// Badge tier sort order (bronze first, locked last)
  ...
  return (
    <div className="min-h-screen pt-12 pb-36 px-6">
      <style>{`
        ...
        .v8-stagger-enter {
          animation: v8-pour-in 0.6s cubic-bezier(0.34, 1.56, 0.64, 1) both;
        }
      ...
      <div className="mx-auto max-w-2xl">
        <div className="text-center mb-8 v8-stagger-enter">
        ...
        <div className="grid grid-cols-3 gap-3 mb-8">
          {[
            ...
            <GlassCard key={stat.label} glowColor={stat.color} className="v8-stagger-enter" style={{ animationDelay: `${50 + i * 60}ms` }}>
        ...
        {sortedBadges.length > 0 && (
          <GlassCard className="mb-6 v8-stagger-enter" glowColor="#f59e0b" style={{ animationDelay: "230ms" }}>
            <div
        ...
        {daygameGrouped.map(({ category, goals }, catIdx) => {
          const catColor = CATEGORY_COLORS[category] ?? "#06b6d4"
          return (
            <div key={category} className="mb-3 rounded-xl overflow-hidden v8-stagger-enter"
              style={{
                ...
                animation: "v8-glow-pulse 5s ease-in-out infinite",
        ...
        {otherAreaData.map(({ area, goals }, areaIdx) => (
          <div key={area.id} className="mb-3 rounded-xl overflow-hidden v8-stagger-enter"
            style={{
        ...
        {(() => {
          ...
          return entries.map(([catId, goals], cIdx) => {
            const catLabel =
              ...
              catId
            return (
              <div key={catId} className="mb-3 rounded-xl overflow-hidden v8-stagger-enter"
                style={{
