...
export function StarCustomizer({
  selectedL1,
  selectedL2s,
  selectedL3s,
  pathColor,
  ...
  return (
    <div className="relative space-y-6">
      ...
      <div>
        <h2 className="text-xl font-bold" style={{ color: "rgba(255,255,255,0.9)" }}>Calibrate your instruments</h2>
        <p className="text-sm mt-1" style={{ color: "rgba(255,255,255,0.45)" }}>
          Fine-tune each orbital body. Toggle them on/off and set their parameters. All settings remain adjustable.
        </p>
      </div>

      {/* Vision goal card — brass frame */}
      <div
      ...
      >
        {/* Corner accents */}
        <div className="absolute top-0 left-0 w-5 h-5 border-t-2 border-l-2" style={{ borderColor: `${brassColor}40` }} />
        <div className="absolute top-0 right-0 w-5 h-5 border-t-2 border-r-2" style={{ borderColor: `${brassColor}40` }} />
        <div className="absolute bottom-0 left-0 w-5 h-5 border-b-2 border-l-2" style={{ borderColor: `${brassColor}40` }} />
        <div className="absolute bottom-0 right-0 w-5 h-5 border-b-2 border-r-2" style={{ borderColor: `${brassColor}40` }} />
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-[10px] font-semibold uppercase tracking-wider" style={{ color: goldColor }}>Polar Star</div>
      ...
      >
        <div className="space-y-1.5">
          {selectedL2s.map((l2) => {
            ...
            if (!cust) return null
            return (
              <div key={l2.id} className="flex items-center gap-3 rounded-lg p-3" style={{ border: `1px solid rgba(205, 133, 63, 0.08)` }}>
                <button
                  ...
                  style={{ background: cust.enabled ? goldColor : "rgba(205, 133, 63, 0.15)" }}
                  role="switch"
                  aria-checked={cust.enabled}
                >
                  <span className={`inline-block size-3.5 rounded-full transition-transform shadow-sm ${cust.enabled ? "translate-x-[18px]" : "translate-x-[3px]"}`} style={{ backgroundColor: cust.enabled ? "#0a0a1a" : "rgba(255,255,255,0.6)" }} />
                </button>
                <div className="flex-1 min-w-0">
                  <span className="text-sm font-medium" style={{ color: cust.enabled ? "rgba(255,255,255,0.85)" : "rgba(255,255,255,0.35)", textDecoration: cust.enabled ? "none" : "line-through" }}>
                    {l2.title}
                  </span>
                </div>
                <Trophy className="size-3.5 flex-shrink-0" style={{ color: "rgba(205, 133, 63, 0.2)" }} />
              </div>
            )
          })}
        </div>
      ...
      {(Object.entries(l3ByCategory) as [GoalDisplayCategory, GoalTemplate[]][]).map(([cat, goals]) => {
        ...
        return (
          <CollapsibleSection
            ...
            icon={<Target className="size-4" style={{ color: "rgba(205, 133, 63, 0.35)" }} />}
            count={enabledInCat}
            total={goals.length}
              ...
              {goals.map((goal) => {
                ...
                return (
                  <div
                    ...
                    style={{
                      border: cust.enabled ? `1px solid rgba(205, 133, 63, 0.08)` : "1px solid rgba(255,255,255,0.03)",
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <span
                            ...
                            style={{
                              color: cust.enabled ? "rgba(255,255,255,0.85)" : "rgba(255,255,255,0.35)",
                              textDecoration: cust.enabled ? "none" : "line-through",
                            }}
                          >
                            {goal.title}
                          </span>
                          <span className="text-[10px] flex-shrink-0 flex items-center gap-1" style={{ color: "rgba(205, 133, 63, 0.3)" }}>
                      ...
                      {cust.enabled && (
                        <div className="flex items-center gap-1.5 flex-shrink-0">
                          <span className="text-xs" style={{ color: "rgba(205, 133, 63, 0.45)" }}>Target:</span>
                          <input
                            type="number"
                            ...
                            style={{
                              backgroundColor: "rgba(205, 133, 63, 0.06)",
                              border: `1px solid rgba(205, 133, 63, 0.15)`,
                              color: "rgba(255,255,255,0.85)",
                            }}
                            min={1}
                          />
                          {isRamp && <span className="text-xs" style={{ color: "rgba(205, 133, 63, 0.35)" }}>/wk</span>}
                        </div>
                      )}
                    </div>
                  </div>
      ...
      >
        <div className="flex items-center justify-between">
          <div className="text-sm" style={{ color: "rgba(255,255,255,0.5)" }}>
            <span className="font-semibold" style={{ color: goldColor }}>{enabledCount}</span> instruments calibrated
          </div>
          <div className="flex items-center gap-3">
            <Button variant="ghost" onClick={onBack} style={{ color: "rgba(205, 133, 63, 0.6)" }}>Back</Button>
            <Button
  ...
  return (
    <div>
      ...
      >
        ...
        {icon}
        <span className="text-sm font-semibold uppercase tracking-wide" style={{ color: "rgba(205, 133, 63, 0.6)" }}>{title}</span>
        <span className="text-xs" style={{ color: "rgba(205, 133, 63, 0.35)" }}>{count}/{total}</span>
        <div className="flex-1 ml-2" style={{ borderTop: "1px solid rgba(205, 133, 63, 0.1)" }} />
      </button>
      {subtitle && isExpanded && (
        <p className="text-xs italic mb-2 ml-6" style={{ color: "rgba(255,255,255,0.3)" }}>{subtitle}</p>
      )}
      {isExpanded && children}
    </div>
  )
