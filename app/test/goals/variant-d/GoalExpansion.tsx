...
const CATEGORY_DESCRIPTIONS: Record<GoalDisplayCategory, string> = {
  ...
  texting: "Text game and conversion metrics",
  dates: "Date planning, escalation, creativity",
...
export function GoalExpansion({ path, onBack, onConfirm }: GoalExpansionProps) {
  ...
  // When L1 is selected, get relevant L2s
  const relevantL2s = useMemo(() => {
    if (!selectedL1) return []
    return tiers.tier2
  }, [selectedL1, tiers.tier2])

  // L3s grouped by category
  ...
  const toggleL2 = (id: string) => {
    setSelectedL2Ids((prev) => {
      ...
      if (next.has(id)) {
        next.delete(id)
        // Also remove L3s that only belong to this L2
        const l3Children = getChildren(id).filter((c) => c.level === 3)
        for (const child of l3Children) {
          // Only remove if no other selected L2 also references this L3
        ...
        next.add(id)
        // Auto-select recommended L3s for this L2
  ...
  const totalSelected = 1 + selectedL2Ids.size + selectedL3Ids.size // L1 + L2s + L3s
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          <div className="flex items-center justify-center size-6 rounded-full text-xs font-bold text-white" style={{ backgroundColor: pathColor }}>
            1
          </div>
          <h2 className="text-lg font-bold">Choose your big goal</h2>
        </div>
        <p className="text-sm text-muted-foreground ml-8">This is your North Star. Everything else builds toward it.</p>
          ...
          {l1Options.map((l1) => {
            ...
            return (
              <button
                ...
                onClick={() => {
                  setSelectedL1(l1)
                  // Auto-select some L2s when switching L1
                  if (!selectedL1 || selectedL1.id !== l1.id) {
                    ...
                    // Auto-select non-dirty-dog L3s for those L2s
                ...
                className={`
                  relative flex items-center gap-3 rounded-xl border-2 p-4 text-left transition-all duration-200 cursor-pointer
                  ${isSelected
                    ? "shadow-md"
      ...
      {selectedL1 && (
        <div className="space-y-3 animate-in fade-in slide-in-from-bottom-2 duration-300">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-bold">Achievements to unlock</h2>
          </div>
          <p className="text-sm text-muted-foreground ml-8">
            These are transformations on the way to "{selectedL1.title}". Select the ones that resonate.
          </p>
            ...
            {relevantL2s.map((l2) => {
              ...
              return (
                <button
                  ...
                  className={`
                    flex items-center gap-3 rounded-lg border p-3 text-left transition-all duration-200 cursor-pointer
                    ${isSelected
                ...
                >
                  ...
                  <div className="min-w-0">
                    <div className="text-sm font-medium truncate">{l2.title}</div>
                    <div className="text-xs text-muted-foreground">{childCount} trackable goals</div>
                  </div>
      ...
      {selectedL1 && selectedL2Ids.size > 0 && (
        <div className="space-y-3 animate-in fade-in slide-in-from-bottom-2 duration-300">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-bold">What you'll track</h2>
          </div>
          <p className="text-sm text-muted-foreground ml-8">
            These are the specific metrics and habits. Toggle off anything that doesn't fit.
          </p>
                      ...
                      {goals.map((goal) => {
                        ...
                        return (
                          <button
                            ...
                            className={`
                              w-full flex items-center gap-3 rounded-md px-3 py-2 text-left transition-all duration-150 cursor-pointer
        ...
        <div className="sticky bottom-0 bg-background/95 backdrop-blur-sm border-t border-border/50 -mx-6 px-6 py-4 mt-6">
          <div className="flex items-center justify-between max-w-3xl mx-auto">
            <div className="text-sm text-muted-foreground">
              <span className="font-semibold text-foreground">{totalSelected}</span> goals selected
              <span className="text-xs ml-2">
                (1 vision + {selectedL2Ids.size} achievements + {selectedL3Ids.size} trackable)
              </span>
            </div>
            <Button
