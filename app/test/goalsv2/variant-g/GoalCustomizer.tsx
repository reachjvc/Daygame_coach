...
export function GoalCustomizer({
  ...
  return (
    <div className="relative space-y-6">
      ...
      <div>
        <h2 className="text-xl font-bold" style={{ color: "rgba(255,255,255,0.9)" }}>Fine-tune your stars</h2>
        <p className="text-sm mt-1" style={{ color: "rgba(255,255,255,0.45)" }}>
          Adjust the brightness of each star. Toggle goals on/off and set targets. Everything is adjustable later.
        </p>
      </div>
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-[10px] font-semibold uppercase tracking-wider" style={{ color: pathColor }}>Your North Star</div>
      ...
      >
        <div className="space-y-1.5">
          {selectedL2s.map((l2) => {
            ...
            if (!cust) return null
            return (
              <div key={l2.id} className="flex items-center gap-3 rounded-lg p-3" style={{ border: "1px solid rgba(255,255,255,0.06)" }}>
                <button
                  ...
                  style={{ background: cust.enabled ? pathColor : "rgba(255,255,255,0.1)" }}
                  role="switch"
                ...
                <Trophy className="size-3.5 flex-shrink-0" style={{ color: "rgba(255,255,255,0.15)" }} />
              </div>
            )
          })}
        </div>
      ...
      {(Object.entries(l3ByCategory) as [GoalDisplayCategory, GoalTemplate[]][]).map(([cat, goals]) => {
        ...
        return (
          <CollapsibleSection
            ...
            icon={<Target className="size-4" style={{ color: "rgba(255,255,255,0.3)" }} />}
            count={enabledInCat}
            total={goals.length}
              ...
              {goals.map((goal) => {
                ...
                return (
                  <div
                    ...
                    style={{
                      border: cust.enabled ? "1px solid rgba(255,255,255,0.06)" : "1px solid rgba(255,255,255,0.03)",
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {cust.enabled && (
                        <div className="flex items-center gap-1.5 flex-shrink-0">
                          <span className="text-xs" style={{ color: "rgba(255,255,255,0.4)" }}>Target:</span>
                          <input
                            type="number"
                            ...
                            min={1}
                          />
                          {isRamp && <span className="text-xs" style={{ color: "rgba(255,255,255,0.35)" }}>/wk</span>}
                        </div>
                      )}
                    </div>
                  </div>
      ...
      >
        <div className="flex items-center justify-between">
          <div className="text-sm" style={{ color: "rgba(255,255,255,0.5)" }}>
            <span className="font-semibold" style={{ color: "rgba(255,255,255,0.85)" }}>{enabledCount}</span> stars configured
          </div>
          <div className="flex items-center gap-3">
            <Button variant="ghost" onClick={onBack} style={{ color: "rgba(255,255,255,0.6)" }}>Back</Button>
            <Button
  ...
  return (
    <div>
      ...
      >
        ...
        {icon}
        <span className="text-sm font-semibold uppercase tracking-wide" style={{ color: "rgba(255,255,255,0.5)" }}>{title}</span>
        <span className="text-xs" style={{ color: "rgba(255,255,255,0.3)" }}>{count}/{total}</span>
        <div className="flex-1 ml-2" style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }} />
      </button>
      {subtitle && isExpanded && (
