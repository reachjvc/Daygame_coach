#!/usr/bin/env bash
# Interleaved batch pipeline — per 20-video chunk EXT(02-05) then LLM(06-09), overlapping
...
#
# Usage: batch-pipeline.sh <BATCH> [ext|llm|both]     e.g. batch-pipeline.sh QT9 both
# Reads chunk manifests docs/pipeline/batches/<BATCH>.<N>.txt (numeric chunks only).
set -uo pipefail
cd /home/jonaswsl/projects/daygame-coach
unset CLAUDECODE
BATCH="${1:?usage: batch-pipeline.sh <BATCH> [ext|llm|both]}"; shift
...
EXTLOG="data/$BATCH.ext.log"
LLMLOG="data/$BATCH.llm.log"
LEDGER="data/$BATCH.settled.txt"   # video ids confirmed-quarantined (clean run, not at 09) — never re-run
touch "$LEDGER"
# Only numeric chunk manifests (<BATCH>.<N>.txt) — NOT the <BATCH>.<N>.inflight.txt files this script creates.
CHUNKS=( $(ls "docs/pipeline/batches/$BATCH".*.txt 2>/dev/null | grep -E "/$BATCH\.[0-9]+\.txt$" | sed -E "s#.*/$BATCH\.([0-9]+)\.txt#\1#" | sort -n) )
[ "${#CHUNKS[@]}" -eq 0 ] && { echo "no chunk manifests docs/pipeline/batches/$BATCH.<N>.txt" >&2; exit 1; }
...
chunk_all_at_05() {  # 0 if every video in chunk N has stage-05 audio-features
  ...
  done < <(grep -v '^#' "docs/pipeline/batches/$BATCH.$n.txt" | grep -oE '\[[A-Za-z0-9_-]+\]$' | tr -d '[]')
  return 0
}

ext_chunk() {  # EXT 02-05 for chunk N; each stage skips videos already done (no --overwrite)
  local n="$1"; local m="docs/pipeline/batches/$BATCH.$n.txt"
  chunk_all_at_05 "$n" && { echo "===== EXT $BATCH.$n all at 05, skip $(date) =====" >> "$EXTLOG"; return; }
  local stages=("02 02.EXT.transcribe" "03 03.EXT.align" "04 04.EXT.diarize" "05 05.EXT.audio-features")
  for s in "${stages[@]}"; do
    set -- $s
    echo "===== EXT $BATCH.$n STAGE $1 $(date) =====" >> "$EXTLOG"
...
clean_outage() {
  local n="$1" id f
  rm -f "data/validation/quarantine/$BATCH.$n.json" "data/validation/quarantine/$BATCH.$n.inflight.json"
  while IFS= read -r id; do
...
inflight_manifest() {  # write LLM-ready not-at-09 videos of chunk N to a .inflight manifest; echo its path (empty if none)
  local n="$1"; local src="docs/pipeline/batches/$BATCH.$n.txt"; local out="docs/pipeline/batches/$BATCH.$n.inflight.txt"; local id line cnt=0
  : > "$out"
...
llm_chunk() {  # LLM 06-09 for chunk N (only not-at-09 videos) with preflight-retry + outage-sleep
  ...
  while :; do
    ...
    [ -z "$mf" ] && { echo "===== LLM $BATCH.$n all settled at 09, skip $(date) =====" >> "$LLMLOG"; return; }
    sub="$BATCH.$n.inflight"
    attempt=$((attempt+1))
    clean_outage "$n"
    tmp=$(mktemp)
    echo "===== LLM $BATCH.$n START (attempt $attempt, $(grep -vc '^#' "$mf" 2>/dev/null||echo '?') in-flight) $(date) =====" >> "$LLMLOG"
    "$RUNNER" "$sub" --manifest "$mf" --from 06 --parallel 5 --skip-end-validation >> "$tmp" 2>&1
    rc=$?
    cat "$tmp" >> "$LLMLOG"
    echo "===== LLM $BATCH.$n EXIT=$rc (attempt $attempt) $(date) =====" >> "$LLMLOG"
    ...
    if [ "$lim" -gt 0 ]; then echo "===== LLM $BATCH.$n quota limit, sleep 30m $(date) =====" >> "$LLMLOG"; sleep 1800; attempt=0; continue; fi
    if [ "$pf" -gt 0 ]; then [ "$attempt" -ge 10 ] && { echo "===== LLM $BATCH.$n GAVE UP preflight $(date) =====" >> "$LLMLOG"; break; }; sleep $(( attempt*30>180?180:attempt*30 )); continue; fi
...
run_ext_all() {
  ...
  echo "===== $BATCH EXT-ALL COMPLETE $(date) =====" >> "$EXTLOG"
}
...
run_llm_all() {
  ...
  echo "===== $BATCH LLM-ALL COMPLETE $(date) =====" >> "$LLMLOG"
}
