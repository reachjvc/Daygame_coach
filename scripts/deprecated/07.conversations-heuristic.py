#!/usr/bin/env python3
"""
scripts/training-data/07.conversations

Conversation Boundary Detection for Daygame Transcripts (LLM / Ollama)

v3.0 - Renamed from 07.LLM-conversations in pipeline consolidation
...
- Input : data/06.segment-enrich/<source>/**/*.json
- Output: data/07.conversations/<source>/**/*.conversations.json

Use:

  A) One source (video / playlist / channel):
     ./scripts/training-data/07.conversations "daily_evolution" "https://www.youtube.com/watch?v=utuuVOXJunM"

  B) Batch from sources file:
     ./scripts/training-data/07.conversations --sources
     ./scripts/training-data/07.conversations --sources docs/sources.txt
        ...
        for attempt in range(retries):
            try:
                ...
                if attempt < retries - 1:
                    time.sleep(1)
                    continue
                print(f"[07.conversations] Ollama error: {e}")
        return None
...
JSON response:"""
        ...
        try:
            ...
            print(f"[07.conversations] Failed to parse LLM response: {e}")
            print(f"[07.conversations] Response preview: {response[:500]}...")
    ...
    def detect_conversations(self, segments: List[Dict]) -> List[Dict]:
        ...
        while idx < total:
            ...
            if (idx % 50) == 0:
                print(f"[07.conversations] Processed {idx}/{total} segments...")
...
def process_file(input_path: Path, output_path: Path, use_llm: bool = True) -> Dict:
    ...
    print(f"[07.conversations] Processing {input_path.name} ({len(segments)} segments)")

    # Step 1: Detect video type (v2.0 improvement)
    video_title = extract_video_title_from_filename(input_path.name)
    print(f"[07.conversations] Video title: {video_title}")

    video_type = detect_video_type_with_llm(segments, video_title)
    print(f"[07.conversations] Detected video type: {video_type}")
    ...
    if video_type in ["talking_head", "podcast"]:
        ...
        print(f"[07.conversations] Non-infield content - forcing all segments to commentary")
    ...
    print(f"[07.conversations] Found {len(conversation_ids)} distinct conversations")
    print(f"[07.conversations] Segment types: {type_counts}")
...
def segment_enrich_root() -> Path:
    return repo_root() / "data" / "06.segment-enrich"


def output_root() -> Path:
    return repo_root() / "data" / "07.conversations"
...
def infer_name_from_input(input_path: Path) -> Optional[str]:
    ...
    stage = parts[i + 1]
    name = parts[i + 2]
    if stage in {"05.audio-features", "06.segment-enrich", "07.conversations"}:
        return name
    return None
...
def main() -> None:
    ...
    def compute_use_llm() -> bool:
        ...
        if use_llm:
            try:
                ...
                if not response.ok:
                    print("[07.conversations] Warning: Ollama not available, using heuristics only")
                    return False
            except requests.exceptions.RequestException:
                print(f"[07.conversations] Warning: Cannot connect to Ollama at {OLLAMA_BASE_URL}")
                return False
        return use_llm
    ...
    def run_detection(input_path: Path, output_path: Path, video_filter: Optional[str], use_llm: bool) -> None:
        if input_path.is_dir():
            ...
            if not files:
                print(f"[07.conversations] No input JSON files found under: {input_path}")
                return

            print(f"[07.conversations] Input : {input_path}")
            print(f"[07.conversations] Output: {output_path}")
            print(f"[07.conversations] Files : {len(files)}")
            ...
            for src in files:
                ...
                print(f"\n[07.conversations] {src.name} -> {dest.name}")
                process_file(src, dest, use_llm=use_llm)
                written += 1

            print("")
            print("[07.conversations] Done.")
        ...
        if dest.exists() and not args.overwrite:
            print(f"[07.conversations] Output exists, skipping: {dest}")
            return
        ...
        print(f"[07.conversations] {input_path} -> {dest}")
    ...
    if args.sources:
        ...
        for src_name, src_url in parse_sources_file(sources_path):
            input_path = segment_enrich_root() / src_name
    ...
    if args.input:
        ...
        if not resolved and name:
            resolved = resolve_existing_path(segment_enrich_root() / args.input)
        if not resolved:
        ...
        input_path = segment_enrich_root() / name
        if not input_path.exists():
            raise SystemExit(f"Expected segment-enrich folder not found: {input_path}")
