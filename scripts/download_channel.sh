...
# NOTE: yt-dlp's --cookies both reads AND writes (dumps) cookies back to the file.
# To avoid clobbering your exported cookies, we always copy to a per-channel runtime file.
...
if [[ -n "$COOKIES_FROM_BROWSER" ]]; then
  ...
  COOKIES_RUNTIME_FILE="$OUTPUT_DIR/.yt-dlp-cookies.runtime.txt"
  cp "$COOKIES_FILE" "$COOKIES_RUNTIME_FILE"
  chmod 600 "$COOKIES_RUNTIME_FILE" 2>/dev/null || true
  COOKIES_ARGS=(--cookies "$COOKIES_RUNTIME_FILE")
  ...
  if ! head -n 1 "$COOKIES_FILE" 2>/dev/null | grep -q "Netscape HTTP Cookie File"; then
    ...
    if grep -q "This file is generated by yt-dlp" "$COOKIES_FILE" 2>/dev/null; then
      COOKIES_NOTE="(looks like yt-dlp overwrote your export; re-export cookies from browser)"
    else
    ...
    if [[ -n "$detected_browser" ]]; then
      ...
      if grep -q "This file is generated by yt-dlp" "$COOKIES_FILE" 2>/dev/null; then
        COOKIES_NOTE="(export clobbered; auto from browser: $detected_browser)"
      else
...
if [[ "${#COOKIES_ARGS[@]}" -gt 0 ]]; then
  if [[ -n "$COOKIES_FROM_BROWSER" ]]; then
    ...
    echo "   Cookies: $COOKIES_FILE -> $COOKIES_RUNTIME_FILE $COOKIES_NOTE"
  fi
...
SAFE_CHANNEL_NAME="$(printf '%s' "$CHANNEL_NAME" | tr -cs 'A-Za-z0-9._-' '_' | sed -E 's/^_+//;s/_+$//')"
...

{
  ...
  (grep -E "ERROR: \\[youtube\\] [A-Za-z0-9_-]{6,}:|WARNING: \\[youtube:tab\\] YouTube said: INFO - [0-9]+ unavailable videos are hidden|WARNING: \\[youtube\\] The provided YouTube account cookies are no longer valid" "$LOG_FILE" || true) \
    | awk -F': ' '
      BEGIN { OFS="\t" }
      /^ERROR: \\[youtube\\] / {
        id=$2
...
if [[ "$DL_STATUS" -ne 0 ]]; then
  ...
  if [[ -z "$COOKIES_FROM_BROWSER" && -f "$COOKIES_FILE" ]] && ! grep -q $'\tLOGIN_INFO\t' "$COOKIES_FILE"; then
    if grep -q "This file is generated by yt-dlp" "$COOKIES_FILE" 2>/dev/null; then
      echo "   Your cookie export looks like it was overwritten by yt-dlp."
    else
