...
describe("trackingRepo Integration Tests", () => {
  ...
  describe("Milestone timing edge cases", () => {
    test("should award night_owl milestone at exactly 21:00 (hour >= 21)", async () => {
      // Arrange
      const userId = await createTestUser()
      await createTestUserStats(userId)
      const client = await getClient()
      ...
      try {
        // Create session that started at exactly 21:00
        await client.query(
          `INSERT INTO sessions (user_id, started_at, is_active)
           VALUES ($1, DATE_TRUNC('day', NOW()) + interval '21 hours', false)`,
          [userId]
        )

        // Act: Check if hour qualifies for night_owl (>= 21)
        const sessionResult = await client.query(
          `SELECT EXTRACT(HOUR FROM started_at) as start_hour
           FROM sessions WHERE user_id = $1`,
          [userId]
        )
        const startHour = parseInt(sessionResult.rows[0].start_hour)

        // Assert: Hour 21 should qualify (>= 21, not > 21)
        expect(startHour).toBe(21)
        expect(startHour >= 21).toBe(true)
        ...
        const milestoneResult = await client.query(
          `SELECT * FROM milestones WHERE user_id = $1 AND milestone_type = 'night_owl'`,
          [userId]
        )
        expect(milestoneResult.rows).toHaveLength(1)
      } finally {
    ...
    test("should award early_bird milestone at 00:00 (midnight edge case)", async () => {
      // Arrange
      const userId = await createTestUser()
      await createTestUserStats(userId)
      const client = await getClient()
      ...
      try {
        // Create session that started at exactly midnight (00:00)
        await client.query(
          `INSERT INTO sessions (user_id, started_at, is_active)
           VALUES ($1, DATE_TRUNC('day', NOW()), false)`,
          [userId]
        )

        // Act: Check if hour qualifies for early_bird (< 10)
        const sessionResult = await client.query(
          `SELECT EXTRACT(HOUR FROM started_at) as start_hour
           FROM sessions WHERE user_id = $1`,
          [userId]
        ...
        // Assert: Hour 0 (midnight) should qualify (< 10)
        expect(startHour).toBe(0)
        expect(startHour < 10).toBe(true)
        ...
        // Verify milestone was awarded
        const milestoneResult = await client.query(
          `SELECT * FROM milestones WHERE user_id = $1 AND milestone_type = 'early_bird'`,
          [userId]
        )
        expect(milestoneResult.rows).toHaveLength(1)
      } finally {
