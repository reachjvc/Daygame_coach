...
- [ ] Verify APPROVE videos copied unchanged with patch_metadata
...
- [ ] Rate limit handling (test Opus limits on R2, fall back to Sonnet if needed)
...
- **Stages renumbered:** Old 09/10 are now 08/09. No stage gap. Pipeline is 01–07 + 06b + 06c (per-source) + 08 (ingest) + 09 (taxonomy report, post-pipeline).
