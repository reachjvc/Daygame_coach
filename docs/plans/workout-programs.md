...
**Resolved 2026-09-05 (corrected 07-09-2026).** On its first run this test found a second orphan: `/preferences/secondary-region`, a working page nothing linked to, while the same setting was already changed inline on the dashboard's "Your Preferences" card (`updateSecondaryRegionDirect` in `UserPreferences.tsx`). Kept for two days on the user's instruction (2026-09-03), then removed in 4bfc843e along with `SecondaryRegionSelector.tsx` and the `updateSecondaryRegion` action; the test is green. An earlier version of this paragraph said the orphan had been "recorded in `.test-known-failures.json` with the reason" — it never was; that file only got a date bump. Left here as a record that a doc can claim a thing the code does not do.
